...
/**
 * [chordpress] – Stu build (minor keys + toolbar assets + power-chord "5" + '+' alias)
...
function cpress_fixed_full_shortcode($atts, $content = '', $tag = 'chordpress') {
    ...
    // Helper to convert chord label to filename (G+ → Gaug.svg, drop /B, # → s)
    $to_filename = function($ch){
        $ch = trim($ch);
        // strip slash bass
        $ch = preg_replace('~/(?:[A-G](?:#|b)?)$~', '', $ch);
        // alias '+' to 'aug' if it's the leading quality
        $ch = preg_replace('/\+(?=[^a-zA-Z]|$)/', 'aug', $ch);
        // ° to dim
        $ch = str_replace('°', 'dim', $ch);
        // # to s
        $ch = str_replace('#', 's', $ch);
        return $ch . '.svg';
    };
        ...
        if (!empty($unique)) {
            foreach (array_keys($unique) as $ch) {
                ...
                $src = file_exists($local_path)
                    ? CPFF_PLUGIN_URL . 'uke-chords/' . $file
                    : content_url('plugins/chordpress/uke-chords/' . $file);
